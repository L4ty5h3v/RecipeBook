...
пользователь в браузере. Тестовые сущности получают уникальный префикс
`__ui_test__ <id>`, поэтому повторные запуски не конфликтуют с уже созданными
данными. После каждого теста данные с этим префиксом удаляются через UI.
